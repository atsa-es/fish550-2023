...
for(i in 1:length(cov.terms)){
  ...
  if (length(cov.terms[ind_on])==0){
    ...
    cov_data =  t(dat_1985[,cov.terms[ind_on]])
      # fill NA values with 0 for now 
  # Check out Ch13 to fill out NA, https://cran.r-project.org/web/packages/MARSS/vignettes/UserGuide.pdf
    cov_data = cov_data %>% replace(is.na(.), 0)
    }
