...
m3 = MARSS(plankton_z, model = list(R = "equalvarcov", m = 3, A = "zero"), control = cont_list, 
                 form = "dfa", z.score = FALSE, covariates = d)
...
for (i in c(2)) {
    fit2 <- MARSS(plankton_z, model = list(R = "diagonal and unequal", m = 3, A = "zero"), 
                control = cont_list, form = "dfa", z.score = FALSE, covariates = d[combo[[i]],])
  
   model.cov.data <- rbind(model.cov.data, data.frame(Covariates = toString(rownames(d)[combo[[i]]]),
...
covariate.table <- cbind(model.cov.data, delta.AICc = model.data$AICc - model.data$AICc[1])
covariate.table
rownames(d)[combo[[16]]]
d
```
