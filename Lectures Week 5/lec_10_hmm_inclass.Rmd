...
Hint: there are a number of ways that you can think about simulating the states, which are discrete (they can be 0 - 1 or 1 - 2). I like to use the `sample()` function, but you can also look at others, including `purrr::rbernoulli()`, `stats::rbinom()`. If you use `rbinom()` or `sample()`, what should you use for the `prob` argument?
...
```{r}
matrix(getpars(fit)[3:6], 2,2, byrow=TRUE)
```
