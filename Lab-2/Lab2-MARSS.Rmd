...
Let's compare to a model without the cycles. Note we cannot compare to `fit1` since that used different data back to 1962. Hmm model without cyles is much better (lower AICc). Even if we only have the 5 year cycles (`covariates[5:6,]`), the AICc is larger than for the model without cycles.
```{r}
fit1.87 <- MARSS(dat87, model=mod.list1, silent=TRUE)
mod.list5 <- list(
  Q = "unconstrained",
  U = "unequal",
  R = "diagonal and equal",
...
fit5.87 <- MARSS(dat87, model=mod.list5, silent=TRUE)
aic <- c(fit1.87$AICc, fit4.87$AICc, fit5.87$AICc)
aic-min(aic)
```
