...
mod.list.0 <- list(B = matrix(1), 
                   U = matrix("u"), #Explore this
                   Q = matrix("q"), #Explore this 
                   Z = matrix(1, 4, 1), #Explore this 
                   A = "scaling", 
...
coef(fit.0)
?coef.marssMLE
#How to select the coefficient in a MARSS model
coef(fit.0, type = "matrix")$R

#If you're having issues fitting/converging, try:

fit.0_test1 <- MARSS(dat, model = mod.list.0, method = "BFGS")

#Also can increase iterations 
