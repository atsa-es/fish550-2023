...
----------------------------------------------------------------
# Fit the model
## we should probably move this section to the very end

Predict historical data for all regions and fit best models

```{r}
# use best model (5) to fit full dataset
fit_full <- MARSS(dat, model=mod.list3, control = list(maxit=1000))
...
Best model is still the best model by a lot
...
But let's go through how we might include cycles. We are going to include cycles with frequency 3, 4, and 5, choose to reflect steelhead returning after 3, 4 or 5 years.

```{r}
TT <- dim(sub_dat)[2] #number of time steps
covariates <- rbind(
  forecast::fourier(ts(1:TT, freq=2), K=1) |> t(),
...
```{r acf-entiat}
fit8 <- MARSS(sub_dat, model=mod.list.cyc)
```
...
df$lag <- as.factor(rep(c(1,5,10), each=3))
df$river <- as.factor(rep(rownames(sub_dat),5))
df$sc <- rep(rep(c("S", "C"), each=3), 5)
...
------------------------------------------------------------------------
## haven't gotten to this, should probably ask in class or office hours
...
# Final section to evaluate AICc


## this includes cycling even though we can't visualize yet
cycling is bad right now but probably because we haven't picked the right cycle so the model is incorporating all of them and that isn't helpful
```{r}
aic <- c(fit3$AICc)
aic-min(aic)
```
