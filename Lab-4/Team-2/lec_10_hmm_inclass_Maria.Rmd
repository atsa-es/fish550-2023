...
y = rnorm(length(x), temp_mean[x], tem_sd[x])
...
y_upi = rnorm(length(x), temp_mean[x], tem_sd[x]) + upwelling_coef*upwelling
...
```{r}
set.seed(121)
fitmod_covariate <- depmix(logit.s ~ 1 + year,
                 nstates = 2,
                 transition = ~ 1,
                 family = gaussian(),
                 data = surv_dat)

summary(fit(fitmod_covariate))
AIC(fitmod_covariate)


fitmod_covariate2 <- depmix(logit.s ~ 1,
                 nstates = 2,
                 transition = ~ 1 + year,
                 family = gaussian(),
                 data = surv_dat)

summary(fitmod_covariate2)
AIC(fitmod_covariate2)
