...
stock_arima = data.frame(matrix(ncol = 5))
names = c("region", "species","p","d","q")
...
stock_gg_long = stock_arima_area %>% 
  pivot_longer(-c(region,species,area), names_to = "parameter", values_to = "param_est")
...
stock_gg_long %>% filter(parameter %in% c("ar1","ar2")) %>%
  ggplot(aes(x=param_est,y=area,color=area)) + geom_boxplot() +
  geom_jitter()+
...
The best fit ARIMA models for AK regions all predict first difference. AK shows no area-wide consensus on the order of the autoregressive part, but strongly supports a first order of the moving average part. ARIMA models for the West Coast regions have evidence for no differencing and a first order on the autoregressive part. WC regions are inconsistent on the predicted order number of the moving area part. 
