...
mod.naknek <- auto.arima(nak.train)
mod.naknek

mod.nush <- auto.arima(nush.train)
mod.nush
...
mod.list <- list(c(0,1,0), c(1,0,1), c(1,0,0))## List of mods to test. 

fits <- list()# list to store the model fits in
err.list <- list(0) # list for accuracy data
for(i in 1:3){
  fits[[i]] <- Arima(nak.train, order = mod.list[[i]])# loops through list, fits model, and stores in 'fits'
...
Nak.test.1 <- forecast(fits[[1]], h=20)
Nak.test.2 <- forecast(fits[[2]], h=20)
Nak.test.3 <- forecast(fits[[3]], h=20)

autoplot(Nak.test.1) + geom_point(aes(x=x, y=y), data=fortify(nak.test))+
  geom_point(aes(x=ret_yr, y=lntotal), data = naknek_fri_fore, col='red')

autoplot(Nak.test.2) + geom_point(aes(x=x, y=y), data=fortify(nak.test))+
  geom_point(aes(x=ret_yr, y=lntotal), data = naknek_fri_fore, col='red')

autoplot(Nak.test.3) + geom_point(aes(x=x, y=y), data=fortify(nak.test))+
  geom_point(aes(x=ret_yr, y=lntotal), data = naknek_fri_fore, col='red')
...
nak.fr.1.2 <- forecast(mod.naknek, h=10) #forecast the data for 10 years
...
residuals(nush.fr) 
fitted(nush.fr) 
forecast::checkresiduals(nush.fr) 
forecast::checkresiduals(nush.fr, plot = FALSE) 
