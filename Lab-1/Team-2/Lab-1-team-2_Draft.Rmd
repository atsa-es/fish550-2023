...
Using the forecast package, we plan to fit ARIMA models to the 1960-2010 data on 4 yr old fish in the Wood and Ugashik systems. Then forecast to 2020. We will then split up the 1.3 and 2.2 age groups and compare. We will measure accuracy by the comparing the RMS error of each model.
...
plot.ts(Ugashik_1.3.ts, ylab = "Log abundance", main = "Time series of sockeye: 1 year in freshwater, 3 years in ocean") 
plot.ts(Ugashik_1.3.ts_out, ylab = "Log abundance", main = "Time series of sockeye: 1 year in freshwater, 3 years in ocean") 
plot.ts(Ugashik_2.2.ts, ylab = "Log abundance", main = "Time series of sockeye: 2 years in freshwater, 2 years in ocean") 
```
...
#check the plot
plot.ts(Wood_1.3.ts, ylab = "log Abundance", main ="Time series of sockeye: 1 year in freshwater, 3 years in ocean" )
#looks like there is a positive trend to the data: not stationary, or could be stationary around a trend
plot.ts(Wood_2.2.ts, ylab = "log Abundance", main ="Time series of sockeye: 2 years in freshwater, 2 years in ocean" )
#looks like it could potentially be stationary

```
***Insert time series comments here

## Kvichak Regional Populations
```{r}
...
#plots
#layout(layout_mat_2)
plot(Kvichak_2.2.ts, ylab = "log Abundance", main = "Time series of sockeye: 2 years in freshwater, 2 years in ocean" )
#looks fairly stationary to me
plot(Kvichak_1.3.ts, ylab = "log(fish counted x1000)", main = "Time series of sockeye: 1 year in freshwater, 3 years in ocean" )
...
tseries::kpss.test(Kvichak_2.2.ts, null = "Trend") #not sure that this test makes sense (talked to Mark about this he said it can still be good to check as the trend could be very subtle), visually data does not appear to have a trend.it fails to reject: stationary
#there is disagreement from adf and kpss, may require differencing
...
##Fitting entire datasets with auto.arima, possibly remove? Or add Ugashik data
...
layout(layout_mat_2)
plot.ts(Wood_2.2.ts, main ="4 year old sockeye returning to Wood River\nthat spent 2 years in Ocean", ylab = "log(counted fish x1000")
lines(fitted(fit.Wood_2.2), col ="red")
plot.ts(Wood_1.3.ts, main ="4 year old sockeye returning to Wood River\nthat spent 3 years in Ocean", ylab = "log(counted fish x1000")
lines(fitted(fit.Wood_1.3), col ="red")
forecast::checkresiduals(fit.Wood_1.3)  #All good news. fail to reject null hypothesis of Ljung-Box, which means the residuals are not autocorrelated. the ACF plot agrees with no significant lags and the histogram looks like a normal distribution
forecast::checkresiduals(fit.Wood_2.2)  #All good news again. fail to reject null of Ljung-Box. no significant lags on ACF. Histogram looks pretty good, two large negative values but otherwise good.
```

```{r}
#fit models
fit.Kvichak_2.2 <- forecast::auto.arima(Kvichak_2.2.ts)
#ARIMA(0,0,1) with non-zero mean. significant pacf lag at 7 could be cyclic nature of date obscuring the MA() pattern
fit.Kvichak_1.3 <- forecast::auto.arima(Kvichak_1.3.ts)
#ARIMA(0,1,1) as expected from acf and pacf plots
#plot
layout(layout_mat_2)
plot(Kvichak_2.2.ts, main= "4 year old sockeye returning to Kvichak River\nthat spent 2 years in ocean", ylab = "log(count of fish x1000)")
lines(fitted(fit.Kvichak_2.2), col = "red")
plot(Kvichak_1.3.ts, main= "4 year old sockeye returning to Kvichak River\nthat spent 3 years in ocean", ylab = "log(count of fish x1000)")
lines(fitted(fit.Kvichak_1.3), col= "red")
#check residuals
...
sigma\^2 = 18.34: log likelihood = -134.93 AIC=273.86 AICc=274.13 BIC=277.56

Series: train_Ugashik_1.3 ARIMA(2,1,2)

Coefficients: ar1 ar2 ma1 ma2 0.6167 -0.9676 -0.8030 0.8451 s.e. 0.0500 0.0341 0.1359 0.1212

sigma\^2 = 1.161: log likelihood = -68.56 AIC=147.11 AICc=148.58 BIC=156.36
...
Series: train_Ugashik_1.3_out ARIMA(2,0,0) with non-zero mean

Coefficients: ar1 ar2 mean 0.3818 -0.5662 6.9256 s.e. 0.1506 0.1451 0.0890
...
sigma\^2 = 1.753: log likelihood = -80.67 AIC=167.34 AICc=167.88 BIC=172.95
...
fr_1.3 <- accuracy(forecast(model_Ugashik_1.3_out, h=10), test_Ugashik_1.3_out) #test accuracy

plot.ts(Ugashik_1.3.ts) # check fit
lines(fitted(model_Ugashik_1.3_out), col="red") #model seems to fit well

forecast::checkresiduals(model_Ugashik_1.3_out) # looks better than 1.3 trained on 1960-2010

autoplot(fc_1.3) + 
  geom_point(aes(x=x, y=y), data=fortify(test_Ugashik_1.3_out)) 
```
...
fr_2.2 <- accuracy(forecast(model_Ugashik_2.2, h=10), test_Ugashik_2.2) #test accuracy

plot.ts(Ugashik_2.2.ts) # check fit
lines(fitted(model_Ugashik_2.2), col="red") #model seems to fit well, follows overall trend as it changes

forecast::checkresiduals(model_Ugashik_2.2) 

autoplot(fc_2.2) + 
  geom_point(aes(x=x, y=y), data=fortify(test_Ugashik_2.2)) 
```
...
plot(pred.Wood_1.3,ylab = "log(fish counted x1000)", xlab = "Time")
...
plot(pred.Wood_2.2,ylab = "log(fish counted x1000)", xlab = "Time")
...
residuals <- (test.Wood_2.2[1:10] - pred.Wood_2.2$mean[1:10])
RMSE.Wood_2.2 <- sqrt(mean(residuals^2))
...
resd.k3 <- test.Kvichak_1.3[1:10] - pred.kvi3$mean[1:10]
...
resd.k2 <- test.Kvichak_2.2[1:10] - pred.kvi2$mean[1:10]
